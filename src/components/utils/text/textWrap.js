/**
 * Utility classes for dynamic SVG text adapted from : https://github.com/jens-ox/vue-data-components/tree/master/src/components/text
 */
...
function set(el, text, config) {
  ...
  const physLn = bbox.height // physical line height
  const unwrappedWidth = bbox.width
  ...
  if (config.align === 'middle') {
    el.setAttribute(
      'transform',
      `translate(${config.paddingLeft}, ${-(el.getBBox().height - physLn) / 2})`
    )
    ...
    el.setAttribute(
      'transform',
      `translate(${config.paddingLeft}, ${-(el.getBBox().height - physLn)})`
    )
  ...
  if (config.scaleToFit) {
    const widthToBoxRatio = w / unwrappedWidth
    if (widthToBoxRatio < 1) {
      el.setAttribute('transform', `scale(${widthToBoxRatio})`)
    }
  }
}
  ...
  const r = {
    inserted(el, binding) {
      if (!(el instanceof SVGTextElement)) {
        throw new Error(
          'Text-wrap directive must be bound to an SVG text element.'
        )
      }
    ...
    update(el, binding, { context }) {
      if (binding.value !== binding.oldValue) {
        ...
        if (binding.value && typeof binding.value !== 'string') {
          ...
          cfg = getConfig(
            {},
            Object.assign({}, el.__WRAP_CONFIG, binding.value)
          )
          set(el, text, cfg)
        } else {
